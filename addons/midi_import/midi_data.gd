...
class Event:
	...
	static func _from_bytes(bytes: PackedByteArray) -> MidiData.Event:
		...
		var consumed_bytes := var_time_data.y + 1

		# Handle running status (no new status byte)
		if status < 0x80:
			status = _last_status
			event_data = bytes.slice(var_time_data.y)
			consumed_bytes = var_time_data.y  # no status byte consumed
		else:
			_last_status = status
		...
		event._full_size = consumed_bytes + event._full_size
...
class NoteOff extends Voice:
	...
	func _init(status: int, bytes: PackedByteArray):
		...
		note = bytes[0] if bytes.size() > 0 else 0
		velocity = bytes[1] if bytes.size() > 1 else 64
		octave = (note / 12) - 1
